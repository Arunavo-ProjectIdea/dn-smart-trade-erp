...
import { archiveDocument, restoreDocument, permanentlyDeleteDocument } from "@/actions/document.actions"
import { Document } from "@/lib/mock-data/documents"
...
export function DocumentsClientView({ initialDocuments }: DocumentsClientViewProps) {
  ...
  const [hardDeleteDialogId, setHardDeleteDialogId] = useState<string | null>(null)
  ...
  const handleDelete = async () => {
    if (deleteDialogId) {
      setIsDeleting(true)
      const res = await archiveDocument(deleteDialogId)
      
      if (res.success) {
        // Change status instead of completely removing it so it shows in Recycle Bin
        setData(data.map(d => d.id === deleteDialogId ? { ...d, status: "Archived" } : d))
        toast({
          title: "Moved to Recycle Bin",
          description: "The document has been successfully moved to the recycle bin.",
        })
      } else {
        toast({
          title: "Error",
  ...
  const handleRestore = async (id: string) => {
    ...
    if (res.success) {
      setData(data.map(d => d.id === id ? { ...d, status: 'Pending Review' } : d))
      toast({ title: "Document Restored", description: "The document has been restored." })
    } else {
      toast({ title: "Error", description: res.error || "Failed to restore document.", variant: "destructive" })
    }
    setIsDeleting(false)
  }

  const handleHardDelete = async () => {
    if (hardDeleteDialogId) {
      setIsDeleting(true)
      const res = await permanentlyDeleteDocument(hardDeleteDialogId)
      if (res.success) {
        setData(data.filter(d => d.id !== hardDeleteDialogId))
        toast({ title: "Document Permanently Deleted", description: "The document has been permanently removed." })
      } else {
        toast({ title: "Error", description: res.error || "Failed to delete document.", variant: "destructive" })
      }
      setIsDeleting(false)
      setHardDeleteDialogId(null)
    }
  }
  ...
  const filteredDocuments = useMemo(() => {
    return data.filter(doc => {
      ...
      // Status filter
      const matchesStatus = statusFilter === "all" ? doc.status !== "Archived" : doc.status === statusFilter
        ...
        <div className="flex h-full items-center justify-center gap-2 whitespace-nowrap flex-nowrap w-[160px]">
          ...
          {userRole !== "Client" && item.status === "Archived" ? (
            <>
              ...
              <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => setHardDeleteDialogId(item.id)} disabled={isDeleting}>Delete</Button>
            </>
          ) : userRole !== "Client" && (
            <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => setDeleteDialogId(item.id)} disabled={isDeleting}>Delete</Button>
          )}
        </div>
      )
    }
  ...
  return (
    <div className="flex flex-col gap-8 pb-10 animate-in fade-in duration-500">
                    ...
                    {userRole !== "Client" && doc.status === "Archived" ? (
                      <>
                        ...
                        <Button
                          ...
                          onClick={() => setHardDeleteDialogId(doc.id)}
                          disabled={isDeleting}
                        >
                          Delete
                        </Button>
      ...
      <ConfirmationDialog 
        ...
        description="Are you sure you want to move this document to the recycle bin? You can restore it later if needed."
        confirmText={isDeleting ? "Moving..." : "Move"}
        variant="destructive"
      ...
      <ConfirmationDialog 
        open={!!hardDeleteDialogId}
        onOpenChange={(open) => !open && setHardDeleteDialogId(null)}
        title="Permanently Delete Document"
        description="Are you sure you want to completely erase this document? This action cannot be undone."
        confirmText={isDeleting ? "Deleting..." : "Permanently Delete"}
        variant="destructive"
        onConfirm={handleHardDelete}
        onCancel={() => setHardDeleteDialogId(null)}
      />
    </div>
  )
}
