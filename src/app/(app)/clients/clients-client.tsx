...
export default function ClientsClient({ initialClients }: { initialClients: Client[] }) {
  ...
  const filteredData = initialClients.filter((client) => {
  ...
  const handleDelete = async () => {
    if (deleteDialogId) {
      const { error } = await deactivateClientAction(deleteDialogId)
      if (error) {
            ...
            <Card key={client.id} className="group relative transition-all duration-300 hover:shadow-card hover:border-border/80">
              <CardHeader className="pb-4">
                ...
                <CardDescription className="flex items-center gap-2">
                  <span className="font-mono text-xs">{formatClientId(client.id)}</span>
